...
class FileProxy:
    ...
    def __init__(self, filename, workspace=None, contents=''):
        ...
        if not isabs(filename) or filename.endswith(sep):
            raise FileNameError('expecting absolute filename')
    ...
    @classmethod
    def from_file(cls, file, workspace, binary=False):
        """
        ...
        if not binary:
            with open(file, 'r',
                      encoding=detect_encoding(file)) as reader:
...
class FileProxyMap:
    ...
    def resolve(self, filename, workspace=None, hard_sync=True, binary=False):
        ...
        except (OSError, FileNameError) as ex:
            if hard_sync:
                raise ex

            # Could raise a FileNameError
